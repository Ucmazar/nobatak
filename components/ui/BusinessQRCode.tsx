...
import { businessPublicURL, isLocalHost, makeBusinessPoster } from '@/lib/business-poster';
...
export function BusinessQRCode({ name, slug, phone }: { name: string; slug: string; phone?: string | null }) {
  ...
  const [result, setResult] = useState({ signature: '', image: '', url: '', local: false, error: '' });
  ...
  useEffect(() => {
    ...
    makeBusinessPoster(name, target, phone).then(image => {
      if (!cancelled) setResult({ signature, image, url: target, local: isLocalHost(new URL(target).hostname), error: '' });
    }).catch(() => {
      if (!cancelled) setResult({ signature, image: '', url: target, local: isLocalHost(new URL(target).hostname), error: 'ساخت برگه انجام نشد. صفحه را دوباره باز کنید.' });
    });
  ...
  return <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 space-y-4" aria-label="کیوآر کسب‌وکار">
    ...
    {current && result.local && <p className="rounded-xl bg-amber-50 border border-amber-200 p-3 text-xs leading-6 text-amber-900">این پیش‌نمایش از آدرس محلی ساخته شده است. برای استفادهٔ مشتریان، وارد سایت آنلاین شوید و برگه را از همان‌جا دریافت کنید.</p>}
